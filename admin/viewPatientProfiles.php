...
if (isset($_POST['save_btn'])) {
    ...
    $sql_insert = "INSERT INTO patient_histories(age, patient_id) VALUES(?, ?)";
    $stmt = $conn->prepare($sql_insert);
    $stmt->bind_param("is", $age, $patient_id);
    
    if($stmt->execute()){
        header("refresh:0");
    }
    else{
        echo "<script>alert('The patient was not added successfully')</script>";
    }
}
?>
